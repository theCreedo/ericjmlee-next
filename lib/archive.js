...
function inferSource(link, id, date) {
  ...
  if (link.includes('outsidethe9to5life.wordpress.com')) return 'wordpress-o9to5'
  return 'external'
}
...
export function getArchiveData({ topic, year } = {}) {
  ...
  return posts.filter((post) => {
    if (post.domains && post.domains.length > 0 && !post.domains.includes('archive')) return false
...
export function getDomainPosts(domainSlug) {
  return getSortedPostsData()
    .filter((post) => Array.isArray(post.domains) && post.domains.includes(domainSlug) && post.title)
    .map((post) => ({
      id: post.id,
      title: post.title,
      date: post.date ? String(post.date).slice(0, 10) : '',
...
export function getAdjacentTopics(slug, allPosts, count = 3) {
  const coCount = {}
  allPosts
    .filter((p) => p.topics.includes(slug))
    .forEach((p) => {
      p.topics.forEach((t) => {
        if (t !== slug) coCount[t] = (coCount[t] || 0) + 1
      })
    })
